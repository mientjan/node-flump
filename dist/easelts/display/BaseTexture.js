...
var BaseTexture = (function () {
    function BaseTexture(source, autoload) {
    ...
    BaseTexture.createFromString = function (source, autoload) {
        if (autoload === void 0) { autoload = false; }
        var img = document.createElement('img');
        img.src = source;
        return new BaseTexture(img, autoload);
    };
    /**
     *
     * @returns {boolean}
     ...
     * @returns {boolean}
     */
    BaseTexture.prototype.isLoading = function () {
        return this._loadPromise != null;
    };
    BaseTexture.prototype.load = function (onProgress) {
        var _this = this;
        if (!this._hasLoaded) {
    ...
    BaseTexture.prototype._load = function (onComplete, onError) {
    ...
    BaseTexture.prototype.initImage = function (image) {
        this.width = image.naturalWidth;
        this.height = image.naturalHeight;
        this._hasLoaded = true;
    };
    BaseTexture.prototype.initCanvas = function (canvas) {
        this.width = canvas.width;
        this.height = canvas.height;
        this._hasLoaded = true;
    };
    BaseTexture.prototype.getWidth = function () {
        return this.width;
    };
    BaseTexture.prototype.getHeight = function () {
        return this.height;
    };
    BaseTexture.prototype.getSource = function () {
        return this.source;
    };
    BaseTexture.prototype.draw = function (ctx, sx, sy, sw, sh, dx, dy, dw, dh) {
        if (this._hasLoaded) {
     ...
     * @returns {boolean}
     */
    BaseTexture.prototype.drawWebGL = function (ctx, sx, sy, sw, sh, dx, dy, dw, dh) {
        //ctx.drawImage( <HTMLImageElement> this.bitmap, sx, sy, sw, sh, dx, dy, dw, dh);
        return true;
    };
    BaseTexture.prototype.bindTexture = function (ctx) {
        var bitmap = this.source;
        if (this.hasLoaded()) {
            if (!(MathUtil_1.MathUtil.isPowerOfTwo(this.width) && MathUtil_1.MathUtil.isPowerOfTwo(this.height))) {
                if (console && console.warn)
                    console.warn("BaseTexture " + this.width + "x" + this.height + " is not power of 2", this);
            }
